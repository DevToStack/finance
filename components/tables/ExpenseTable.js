...
export default function ExpenseTable() {
    ...
    const categories = [...new Set(expenses.map(i => i.category))];
    const totalExpense = filteredExpenses.reduce((sum, i) => sum + parseFloat(i.amount), 0);
    ...
    const handleSelectAll = () => {
        if (selectedRows.length === paginatedExpenses.length) {
            setSelectedRows([]);
        } else {
            setSelectedRows(paginatedExpenses.map(i => i.transaction_id));
        }
    };
    ...
    const exportToCSV = () => {
        ...
        const csvData = filteredExpenses.map(i => [
    ...
    // Get selected expense details for the modal
    const getSelectedExpenseDetails = () => {
        const selectedExpenses = expenses.filter(e => selectedRows.includes(e.transaction_id));
        const totalAmount = selectedExpenses.reduce((sum, e) => sum + parseFloat(e.amount), 0);
        return { selectedExpenses, totalAmount };
    };

    // Dynamic classes based on theme
    const statCard = {
        total: isDark ? 'bg-gradient-to-br from-red-950/30 to-rose-950/30 border-red-800' : 'bg-gradient-to-br from-red-50 to-rose-50 border-red-100',
        totalText: isDark ? 'text-red-300' : 'text-red-700',
        totalValue: isDark ? 'text-red-200' : 'text-red-700',
        totalIconBg: isDark ? 'bg-red-900/30' : 'bg-red-100',
        totalIcon: isDark ? 'text-red-400' : 'text-red-600',

        transactions: isDark ? 'bg-gradient-to-br from-orange-950/30 to-amber-950/30 border-orange-800' : 'bg-gradient-to-br from-orange-50 to-amber-50 border-orange-100',
        transactionsText: isDark ? 'text-orange-300' : 'text-orange-700',
        transactionsValue: isDark ? 'text-orange-200' : 'text-orange-700',
        transactionsIconBg: isDark ? 'bg-orange-900/30' : 'bg-orange-100',
        transactionsIcon: isDark ? 'text-orange-400' : 'text-orange-600',
    ...
    const inputFocus = isDark ? 'focus:ring-red-600' : 'focus:ring-red-500';
    const categoryBtnActive = isDark ? 'bg-red-700 text-white' : 'bg-red-500 text-white';
    ...
    const tableRowHover = isDark ? 'hover:bg-gray-700/50' : 'hover:bg-gray-50';
    ...
    const categoryBadge = isDark ? 'bg-red-900/30 text-red-300' : 'bg-red-50 text-red-700';
    const amountText = isDark ? 'text-red-400' : 'text-red-600';
    ...
    const paginationBg = isDark ? 'bg-gray-950/50 border-gray-900' : 'bg-gray-50 border-gray-200';
    ...
    const paginationActive = isDark ? 'bg-red-700 text-white' : 'bg-red-500 text-white';
    ...
    const modalBorder = isDark?'border-gray-800':'border-gray-100';
    ...
    const modalSummaryBg = isDark ? 'bg-gray-700/50' : 'bg-gray-50';
    const modalSummaryText = isDark ? 'text-gray-300' : 'text-gray-600';
    ...
    return (
        <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                <div className={`${statCard.total} rounded-xl p-4 border transition-all duration-300`}>
                    <div className="flex items-center justify-between">
                        <div>
                            <p className={`text-xs ${statCard.totalText} font-medium uppercase tracking-wide`}>Total Expenses</p>
                            <p className={`text-2xl font-bold ${statCard.totalValue} mt-1`}>{formatCurrency(totalExpense)}</p>
                        </div>
                ...
                <div className={`${statCard.transactions} rounded-xl p-4 border transition-all duration-300`}>
                    <div className="flex items-center justify-between">
                        <div>
                            ...
                            <p className={`text-2xl font-bold ${statCard.transactionsValue} mt-1`}>{filteredExpenses.length}</p>
                        </div>
                        <div className={`w-10 h-10 ${statCard.transactionsIconBg} rounded-full flex items-center justify-center`}>
                            <FiTrendingDown className={`w-5 h-5 ${statCard.transactionsIcon}`} />
                        </div>
                    </div>
                </div>
                ...
                <div className={`${statCard.average} rounded-xl p-4 border transition-all duration-300`}>
                    <div className="flex items-center justify-between">
                        <div>
                            <p className={`text-xs ${statCard.averageText} font-medium uppercase tracking-wide`}>Average Expense</p>
                            <p className={`text-2xl font-bold ${statCard.averageValue} mt-1`}>{formatCurrency(averageExpense)}</p>
                        </div>
                        <div className={`w-10 h-10 ${statCard.averageIconBg} rounded-full flex items-center justify-center`}>
                            <FiShoppingBag className={`w-5 h-5 ${statCard.averageIcon}`} />
                        </div>
                    </div>
                </div>
            </div>
                        ...
                        <thead className={`${tableHeaderBg} border-b transition-colors duration-300`}>
                            <tr>
                                <th className="w-10 px-4 py-3">
                                    <input
                                        type="checkbox"
                                        checked={selectedRows.length === paginatedExpenses.length && paginatedExpenses.length > 0}
                        ...
                        <tbody className={`divide-y ${tableBorder}`}>
                            {paginatedExpenses.length === 0 ? (
                                <tr>
                                    <td colSpan="6" className="text-center py-12">
                                        <div className="flex flex-col items-center justify-center">
                                            ...
                                            <p className={`font-medium ${emptyStateTitle}`}>No expense entries found</p>
                                ...
                                paginatedExpenses.map(expense => (
                                    <tr key={expense.transaction_id} className={`${tableRowHover} transition-colors duration-200`}>
                                        <td className="px-4 py-4">
                                            <input
                                                type="checkbox"
                                                checked={selectedRows.includes(expense.transaction_id)}
                                                onChange={() => handleSelectRow(expense.transaction_id)}
                                                className="w-4 h-4 rounded border-gray-300 dark:border-gray-600 text-red-600 focus:ring-red-500"
                                            />
                                        </td>
                                        <td className="px-6 py-4">
                                            ...
                                            <div className={`text-xs ${tableIdText} mt-0.5`}>
                                                ID: {formatTransactionId(expense.transaction_id)}
                                            </div>
                                        </td>
                                        <td className="px-6 py-4">
                                        ...
                                        <td className="px-6 py-4">
                                            <div className="flex items-center justify-center gap-2">
                                                ...
                                                <button
                                                    onClick={() => setShowDeleteConfirm(expense.transaction_id)}
                ...
                <div className={`fixed inset-0 ${modalOverlay} flex items-center justify-center z-50 animate-fadeIn`}>
                    <div className={`${modalBg} ${modalBorder} border rounded-xl p-6 max-w-md mx-4 shadow-xl transition-all duration-300`}>
                        <div className="text-center">
                            ...
                            <h3 className={`text-lg font-semibold ${modalTitle} mb-2`}>Delete Expense Entry?</h3>
            ...
            {showBulkDeleteConfirm && (
                <div className={`fixed inset-0 ${modalOverlay} flex items-center justify-center z-50 animate-fadeIn`}>
                    <div className={`${modalBg} rounded-xl p-6 max-w-md mx-4 shadow-xl transition-all duration-300`}>
                        <div className="text-center">
                            <div className={`w-12 h-12 ${modalIconBg} rounded-full flex items-center justify-center mx-auto mb-4`}>
                                <FiTrash2 className={`w-6 h-6 ${modalIcon}`} />
                            </div>
                            <h3 className={`text-lg font-semibold ${modalTitle} mb-2`}>Delete Multiple Expenses?</h3>
                            <p className={`text-sm ${modalText} mb-4`}>
                                Are you sure you want to delete <span className="font-semibold text-red-600 dark:text-red-400">{selectedRows.length}</span> expense entries?
                            </p>

                            {/* Show summary of selected expenses */}
                            <div className={`${modalSummaryBg} rounded-lg p-3 mb-6 max-h-40 overflow-y-auto`}>
                                <div className={`text-xs ${modalSummaryText} space-y-1`}>
                                    {getSelectedExpenseDetails().selectedExpenses.slice(0, 5).map(expense => (
                                        <div key={expense.transaction_id} className="flex justify-between items-center py-1">
                                            <span className="truncate max-w-[150px]">{expense.description || 'No description'}</span>
                                            <span className="font-medium text-red-600 dark:text-red-400">-{formatCurrency(expense.amount)}</span>
                                        </div>
                                    ))}
                                    {getSelectedExpenseDetails().selectedExpenses.length > 5 && (
                                        <div className="text-center text-gray-400 dark:text-gray-500 pt-1">
                                            +{getSelectedExpenseDetails().selectedExpenses.length - 5} more items
                                        </div>
                                    )}
                                </div>
                            </div>
                            ...
                            <div className="flex justify-between items-center mb-6 px-2">
                                <span className={`text-sm font-medium ${modalText}`}>Total Amount:</span>
                                <span className="text-lg font-bold text-red-600 dark:text-red-400">
                                    -{formatCurrency(getSelectedExpenseDetails().totalAmount)}
                                </span>
                            </div>
