...
export default function AddIncomeForm({ onClose }) {
    ...
    const inputFocusRing = isDark ? 'focus:ring-green-600' : 'focus:ring-green-500';
    ...
    const submitButtonBg = isDark ? 'bg-green-600 hover:bg-green-700' : 'bg-green-500 hover:bg-green-600';
    ...
    return (
        <div className={`fixed inset-0 ${overlayBg} flex items-center justify-center z-50`}>
            <div className={`${modalBg} rounded-xl w-full max-w-md mx-4 shadow-xl`}>
                <div className={`flex justify-between items-center p-6 border-b ${borderColor}`}>
                    <h3 className={`text-xl font-semibold ${textPrimary}`}>Add Income</h3>
                    <button
                        onClick={onClose}
                        className={`p-1 ${closeButtonHover} rounded-lg  `}
                    >
                ...
                <form onSubmit={handleSubmit} className="p-6 space-y-4">
                    <div>
                        ...
                        <input
                            ...
                            className={`w-full px-3 py-2 ${inputBg} border rounded-lg focus:ring-2 ${inputFocusRing} focus:outline-none  `}
                            placeholder="0.00"
                        />
                    ...
                    <div className="flex gap-3 pt-4">
                        <button
                            type="button"
                            onClick={onClose}
                            className={`flex-1 px-4 py-2 border rounded-lg ${cancelButtonBg} ${cancelButtonText}  `}
                        >
                            Cancel
                        </button>
                        <button
                            type="submit"
                            className={`flex-1 px-4 py-2 ${submitButtonBg} text-white rounded-lg  `}
                        >
                            Add Income
                        </button>
                    </div>
