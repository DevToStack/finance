...
export default function Sidebar() {
    ...
    // Desktop Sidebar - Now relative positioned
    return (
        <>
            {/* Desktop Sidebar - Relative positioned, no longer fixed */}
            <aside
                className={`hidden md:block bg-[hsl(var(--card))] border-r border-[hsl(var(--border))] transition-all duration-300 overflow-hidden flex-shrink-0 ${isCollapsed ? 'w-20' : 'w-64'
                    }`}
            >
                <div className="flex flex-col h-full sticky top-0">
                    <div className="flex items-center justify-between px-4 h-16 border-b border-[hsl(var(--border))] flex-shrink-0">
                        ...
                        <button
                            ...
                            className="p-1 rounded-lg hover:bg-[hsl(var(--muted))] flex-shrink-0"
                        >
                        ...
                        <div className="space-y-1">
                            {menuItems.map((item) => {
                                ...
                                return (
                                    ...
                                    >
                                        {active && (
                                            <div className="absolute left-0 top-1/2 -translate-y-1/2 w-1 h-6 bg-blue-500 rounded-r"></div>
                                        )}
                    ...
                    <div className="border-t border-[hsl(var(--border))] px-3 py-4 flex-shrink-0">
                        ...
                        {!isCollapsed && (
                            <div className="space-y-1 mt-2">
                                <Link href="/dashboard/settings" className="flex items-center px-3 py-3 rounded-lg text-sm font-bold text-[hsl(var(--muted-foreground))] hover:bg-[hsl(var(--muted))] hover:text-[hsl(var(--foreground))] transition-all duration-200">
                                    <FiSettings className="w-5 h-5 mr-3" />
                                    <span>Settings</span>
                                </Link>
                                <button className="w-full flex items-center px-3 py-3 rounded-lg text-sm font-bold text-red-500 hover:bg-red-50 dark:hover:bg-red-950/30 transition-all duration-200">
            ...
            {/* Mobile Top Bar */}
            <div className="md:hidden fixed top-0 left-0 right-0 bg-[hsl(var(--card))] border-b border-[hsl(var(--border))] h-16 px-4 flex items-center justify-between z-40">
                <button
            ...
            <div
                className={`fixed inset-0 z-50 md:hidden transition-all duration-300 ${isMobileOpen ? 'visible' : 'invisible'
                    }`}
            >
                <div
                    className={`absolute inset-0 bg-black transition-opacity duration-300 ${isMobileOpen ? 'opacity-50' : 'opacity-0'
                        }`}
                ...
                <div className={`absolute top-0 left-0 h-full w-64 bg-[hsl(var(--card))] shadow-xl transition-transform duration-300 ease-out ${isMobileOpen ? 'translate-x-0' : '-translate-x-full'
                    ...
                    <div className="flex flex-col h-full">
                        <div className="flex items-center justify-between px-4 h-16 border-b border-[hsl(var(--border))] flex-shrink-0">
                            ...
                            <button
                                onClick={() => setIsMobileOpen(false)}
                                className="p-1 rounded-lg hover:bg-[hsl(var(--muted))]"
                            >
                        ...
                        <div className="border-t border-[hsl(var(--border))] px-3 py-4 flex-shrink-0">
                            <Link
                                ...
                                onClick={() => setIsMobileOpen(false)}
                                className="flex items-center space-x-3 px-3 py-3 rounded-lg text-sm font-bold text-[hsl(var(--muted-foreground))] hover:bg-[hsl(var(--muted))] hover:text-[hsl(var(--foreground))]"
                            >
                                <FiSettings className="w-5 h-5" />
                                <span>Settings</span>
                            </Link>
                            <button className="w-full flex items-center space-x-3 px-3 py-3 rounded-lg text-sm font-bold text-red-500 hover:bg-red-50 dark:hover:bg-red-950/30">
